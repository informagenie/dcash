...
<div class="content-wrapper">
    <section class="content-header">
        <h1>
            Commande
            <small>Détail sur la commande <strong>#<?= $options->reference ?></strong> <span
                    style="background: <?= status_name($options->status)['color'] ?>"
                    class="badge"><?= status_name($options->status)['name'] ?></span></small>
        </h1>
    </section>
    <section class="content">
        <div class="row">
            <div class="col-md-6">
                <form method="post" action="<?= site_url('orders') ?>">
                    <select required class="form-control" id="items-select" name="items">
                        ...
                        <?php foreach ($items as $item => $value): ?>
                            <optgroup label="<?= $item ?>">
                                <option value="<?= urlencode($item) ?>">Produit
                                    #<?= current($value)['item_product_id'] ?>(<?= total_item($items[$item]) ?>$)
                                </option>
                            </optgroup>
                        <?php endforeach ?>
                    </select>
                    ...
                    <?php endforeach ?>
                    <input <?php if ($options->status == STATUS_END) echo 'readonly disabled title="Les paiments pour cette commande ont été déjà effectués"'; ?>
                        autocomplete="off" class="form-control" placeholder="Entrer le numéro de référence"
                        maxlength="10" required pattern="[a-zA-Z0-9]{10}" type="text" name="__ref">
                    <button <?php if ($options->status == STATUS_END) echo 'readonly disabled' ?> type="submit"
                ...
                <legend><a href="<?= unserialize($options->options)['return'] ?>" target="_blank">Voir la commande</a>
                </legend>
            </div>
            <div class="col-md-6">
                    ...
                    <div class="list-group">
                        ...
                        <?php foreach ($items_delivred as $item): ?>
                            <li class="list-group-item list-group-item-success">
                                <?= get_instance()->getUserInfo($item->vendor_id)['name'] ?> (email:
                                <em><?= get_instance()->getUserInfo($item->vendor_id)['email'] ?></em> réf: <a
                                    href="<?= site_url('check?__ref=' . $item->reference) ?>">#<?= $item->reference ?></a>)
            ...
            <div class="col-md-6">
                <div class="list-group">
                    ...
                    <span class="list-group-item">
                        ...
                        <span
                            class="badge"><?= total_items(get_grouped_item(get_items(unserialize($options->options)))) ?> <?= devise_name($options->devise) ?></span>
                    </span>
                    <span class="list-group-item">
                ...
                <? //= $item  ?><!--<span class="badge"><span class="glyphicon glyphicon-ok"></span></span></li>-->
                <!--                        --><?php //endforeach  ?>
                <!--                    </div>-->
                <!--                --><?php //else:  ?>
                <!--                    <div class="alert alert-warning">Aucun etablissement payé pour cette commande</div>-->
