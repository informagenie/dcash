...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            ...
            <div data-toggle="dismiss" class="alert-dismissible alert alert-success">Vous êtes en mode Administrateur. Vous pouvez tout voir mais pas modifier ou agir pour l'instant</div>
            ...
            <div class="col-lg-3 col-xs-6">
                ...
                <div class="small-box bg-green">
                    <div class="inner">
                        <h3><?= (isset($price['success'])) ?  (int)__($price['success']) : (int)__($price['end']) ?>$</h3>
                        <p>Vérifiés</p>
                    </div>
                    <div class="icon">
        ...
        <div class="row">
                    ...
                    <div class="box-body">
                        ...
                        <div class="table-responsive table-responsive">
                            <table class="table table-condensed table-bordered">
                                ...
                                <tbody>
                                <?php $status = 0; foreach($payments[$sort] as $payment): ?>
                                    <tr>
                                        <td>#<a href="<?= site_url('commande/'.$payment->reference) ?>"><?= $payment->reference ?></a></td>
                                            ...
                                            <?php foreach(status_list() as $s=>$v):?>
                                                <div class="col-md-1">
                                                    <form method="post" action="<?= site_url('update') ?>">
                                                        ...
                                                        <button <?php if($payment->status == $s) echo 'disabled' ?> type="submit" title="<?= $v['name'] ?>" class="btn btn-<?= $v['btn_type'] ?>"><span class="glyphicon <?= $v['icon'] ?>"></span></button>
                                                    </form>
                                                </div>
                                            <?php endforeach ?>
                                        </td>
                ...
                <?php else:  ?>
                    ...
                    <?php foreach($payments as $payment=>$value):  ?>
                        <tr>
                            ...
                            <td><a href="<?= site_url('check?__ref='.$value->reference) ?>">Plus d'informations</a></td>
                        </tr>
                    <?php endforeach  ?>
                <?php endif  ?>
                </table>
            ...
            <?php if($role == ROLE_LIVREUR):  ?>
                ...
                <?php else:  ?>
                    ...
                    <?php $index = 1; foreach($payments as $item=>$detail):  ?>
                        <tr>
                            <td>
                                <?= $index  ?>
                            </td>
